...
git clone https://github.com/efebagri/paste-server.git
cd pasteserver
...
## 🔄 Storage Failover
The PasteServer includes an automatic failover mechanism:

- If the selected database connection fails (e.g., MySQL cannot connect), the server automatically falls back to file storage
- This ensures high availability even when your database is temporarily down
- Error logs will indicate when a fallback occurs: ``Failed to initialize 'mysql' storage: [error]`` followed by ``Switching to fallback: File Storage...``
- Make sure ``STORAGE_PATH` is configured to ensure file storage works correctly in fallback mode
...
| ✅ | Storage Fallback | Automatic fallback to file storage | 🗄️ All |
